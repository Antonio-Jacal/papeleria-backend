...
func GenerateNextNumeroLista(collection *mongo.Collection) (string, error) {
	...
	opts := options.FindOne().
		SetSort(bson.D{{Key: "numeroLista", Value: -1}}).
		SetProjection(bson.M{"numeroLista": 1}) // <- solo traemos este campo

	filter := bson.M{
		"numeroLista": bson.M{"$regex": "^LTS\\d{4}$"},
	}

	// struct chico, solo para este propósito: no le importa el resto del documento
	var lastList struct {
